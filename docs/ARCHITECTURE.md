...
scripts/:`run_pipeline.py`(盘后主管道 14 步) `run_light.py`(盘中轻量) + 6 个编排 sh(盘前/盘中/盘后/美股/信函/成绩单,**flock 全局串行锁+lib_alert 旗标**) + 台北侧采集 fetch_*(us_board/us_overnight/tech_wire/fund_letters/build_us) + 运维(backup_db/manage_ledger/manage_holdings/source_status/init_db)。
...
导航 7 页:**报告**(大盘仪表→主线hero→三件事→**B6节点研判**→**B8个股决策[校准横幅]**→证伪→盘中节奏→综述;右栏:资金面/隔夜美股/**B7成绩单**/账本/事件流) / **热点**(排名+X舆情+全量新闻流下钻) / **资金**(当日曲线|多日趋势) / **热力**(四象限+时间窗+下钻) / **研究** / **信函** / **系统**(health+信源面板)。顶部 A股|美股切换;任意股票可点开详情弹层(6M走势);手机 md: 断点适配。
