  ...
  return (
    <div className="max-w-4xl space-y-3">
      <div className="flex items-center gap-2 text-[14px]">
        ...
        <span className="text-dim text-[13px]">目标价共识 / 评级 / 覆盖分析师数(客观事实,非本系统判断)</span>
        <div className="ml-auto flex gap-1 text-[13px]">
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.code} onClick={() => open({ code: r.code, name: r.name })} className="border-t hairline cursor-pointer hover:bg-elevated/40">
              ...
              <td>
                {r.rec_key && <span className={`px-1.5 py-0.5 rounded text-[13px] ${recColor(r.rec_key)}`}>{recLabel[r.rec_key] || r.rec_key}</span>}
              </td>
