  ...
  return (
    <div className="max-w-4xl space-y-3">
      <div className="flex items-center gap-2 text-[13px]">
        ...
        <span className="text-dim text-[12px]">目标价共识 / 评级 / 覆盖分析师数(客观事实,非本系统判断)</span>
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.code} className="border-t hairline">
              ...
              <td className="mono text-dim text-[12px]">{r.code}</td>
              ...
              <td>
                {r.rec_key && <span className={`px-1.5 py-0.5 rounded text-[12px] ${recColor(r.rec_key)}`}>{recLabel[r.rec_key] || r.rec_key}</span>}
              </td>
