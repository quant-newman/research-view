...
function Bar52({ p }: { p: number | null }) {
  if (p == null) return <span className="text-dim text-[11px]">—</span>;
  return (
    <div className="flex items-center gap-1 justify-end">
      ...
      <span className="text-dim text-[10px] w-6 text-right">{p}</span>
    </div>
  );
}
...
function Row({ it }: { it: UsBoardItem }) {
  return (
    <tr className="border-t hairline">
      ...
      <td className="mono text-dim text-[11px]">{it.ticker}</td>
...
export function UsBoardView({ b }: { b: UsBoard | null | undefined }) {
  ...
  if (!b || !items.length) {
    return (
      <div className="text-muted p-4 text-[13px]">
  ...
  return (
    <div className="space-y-3 max-w-4xl">
      <div className="flex flex-wrap items-center gap-3 text-[12px]">
        ...
        <span className="text-dim text-[11px]">领先指标外盘 · 红涨绿跌</span>
        ...
        <div className="space-y-4">
          {groups.map(([sector, its]) => (
            <div key={sector}>
              ...
              <table className="w-full text-[12px]">{HEAD}<tbody>{its.map((it) => <Row key={it.ticker} it={it} />)}</tbody></table>
            </div>
          ))}
        </div>
      ) : (
        <table className="w-full text-[12px]">{HEAD}<tbody>{flat.map((it) => <Row key={it.ticker} it={it} />)}</tbody></table>
      )}
    </div>
  );
}
