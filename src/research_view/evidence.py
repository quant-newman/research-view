...
# user prompt 模板(规则文本,静态):prompt_hash = sha256(SYSTEM+本模板+lessons段)前16位,
# {date}/{blocks} 每日数据不入哈希——否则天天变,B7 没法按 prompt 版本分组校准样本。
_USER_TMPL = """下面是今日({date})A股AI科技产业链各节点的六源证据矩阵(已按共振分绝对值排序)。
z = 该源指标在全部产业链节点截面的标准分(衡量相对全池强弱,|z|≥1为显著);共振分 = 方向源(新闻/资金/行情/龙虎榜/信函)加权z和,由代码算出仅供参考——你可以不同意共振方向,但必须在 thesis 里说明理由。
...
def _lessons_seg() -> str:
    """B7 校准回路:最新一份非空 lessons 注入 prompt(经验层,不是事实源)。
    校准期冻结(DECISIONS #28):只落库不注入,防首批样本 prompt 漂移。"""
    if config.calibration_freeze():
        return ""
    try:
        ...
        for c in cards:
            cur.execute("""INSERT INTO judgment_card(trade_date,node_id,direction,confidence,
                ...
                (date_utc8, c["node_id"], c["direction"], c["confidence"], c["subjective_prob"],
                 ...
                 json.dumps(c["divergence"], ensure_ascii=False), model, c["prompt_hash"]))
    return len(cards)
