...
def _signals(date_utc8: str, top: int = 10) -> list[dict]:
    ...
    with db.rv_conn() as conn, conn.cursor() as cur:
        ...
        for nid, txt, _se in cur.fetchall():
            node_news.setdefault(nid, []).append(txt)
        # 个股→节点 + 代表票名
...
规则:items 顺序与条数尽量对应输入(可略去信号极弱的);reason 只陈述事实,禁止"看好/建议买入/值得关注"等判断词;trend 用输入里"今日vs昨日新闻数"参考。
【信号】
{chr(10).join(blocks)}"""
    try:
    ...
    return {"headline": j.get("headline") or "今日主题热度", "items": items}


def persist(date_utc8: str) -> int:
    out = generate(date_utc8)
    with db.rv_conn() as conn, conn.cursor() as cur:
        cur.execute("""INSERT INTO hotspot_daily(report_date, headline, items)
            VALUES(to_date(%s,'YYYYMMDD'), %s, %s)
            ON CONFLICT(report_date) DO UPDATE SET headline=EXCLUDED.headline,
              items=EXCLUDED.items, generated_at=now()""",
            (date_utc8, out["headline"], json.dumps(out["items"], ensure_ascii=False)))
