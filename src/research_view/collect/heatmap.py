...
def _returns(bars: dict[str, list[tuple]]) -> dict[str, tuple[float | None, float | None]]:
    """每 ts_code 的 (1M, 6M) 涨幅%。bars: ts_code -> [(date, close)...] 升序。"""
    out = {}
    for ts, series in bars.items():
        if len(series) < 2:
            out[ts] = (None, None); continue
...
def compute() -> dict[str, int]:
    ...
    for ts in pool_ts:
        ...
        code = ts2code[ts]
        stock_rows.append((code, ts, ts2name[ts], mv, pe, ps, r1, r6, or_yoy, gm, np_yoy, pe_pct))
        for nid in code_nodes.get(code, []):
            per_node.setdefault(nid, []).append({"mv": mv, "pe": pe, "ps": ps, "r1": r1,
                                                 "r6": r6, "or_yoy": or_yoy, "gm": gm})
    ...
    for nid, items in per_node.items():
        ...
        node_agg[nid] = {
            ...
            "r1": med(items, "r1"), "r6": med(items, "r6"), "or_yoy": med(items, "or_yoy"),
    ...
    node_rows = [(nid, n["chain"], n["node"], n["n"], n["mv"], n["r1"], n["r6"],
    ...
    with db.rv_conn() as conn, conn.cursor() as cur:
        ...
        cur.executemany("""INSERT INTO heatmap_stock(code,ts_code,name,total_mv,pe,ps,ret_1m,ret_6m,
            or_yoy,gross_margin,netprofit_yoy,pe_pct) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""", stock_rows)
        cur.executemany("""INSERT INTO heatmap_node(node_id,chain,node,n_stocks,total_mv,ret_1m,ret_6m,
            or_yoy,gross_margin,pe,ps,quadrant) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""", node_rows)
